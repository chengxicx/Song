...
# Review
#
# These read the live DOM rather than the served HTML.  The criteria
# builder is generated by lute-review-criteria.js after the document
# loads, and a regression once had it read its JSON config at script
# load time -- before the form partial rendered those tags.  The builder
# silently stayed empty and every unit test still passed, because the JS
# harness stubs getElementById.  Only a real page can tell us otherwise.


@then(parsers.re(r"the review builder shows (?P<count>\d+) conditions?"))
def then_review_condition_count(luteclient, count):
    "One row per condition; zero means the builder never ran."
    state = luteclient.review_builder_state()
    assert state["builder_present"], f"no criteria builder on the page: {state}"
    assert state["conditions"] == int(count), state


@then(parsers.parse('the review builder shows the preset "{label}"'))
def then_review_preset(luteclient, label):
    state = luteclient.review_builder_state()
    assert state["builder_present"], f"no criteria builder on the page: {state}"
    assert state["preset"] == label, state


@then(parsers.parse('the review criteria are "{criteria}"'))
def then_review_criteria(luteclient, criteria):
    "Both views of the criteria: the preview, and the field that submits."
    state = luteclient.review_builder_state()
    assert state["preview"] == criteria, state
    assert state["textarea"] == criteria, state


@when(parsers.parse('I choose the review preset "{label}"'))
def when_choose_review_preset(luteclient, label):
    luteclient.choose_review_preset(label)


@when(parsers.parse('I set review condition {position:d} to "{value}"'))
def when_set_review_condition(luteclient, position, value):
    luteclient.set_review_condition_value(position, value)


@when(parsers.parse('I save the review spec named "{name}"'))
def when_save_review_spec(luteclient, name):
    luteclient.save_review_spec(name)


@then(parsers.parse('the review spec list shows "{name}" with criteria "{criteria}"'))
def then_review_spec_row(luteclient, name, criteria):
    table = luteclient.get_review_spec_table()
