...
class LuteTestClient:  # pylint: disable=too-many-public-methods
    ...
    def review_builder_state(self):
        """
        The criteria builder's live DOM state, as plain data.

        Ask the page, not the markup: the builder is generated by
        lute-review-criteria.js after the document loads, so whether it
        ran at all only shows up in the live DOM.  Reading the served
        HTML would happily report a form whose builder never started.
        """
        return self.page.evaluate(
            """() => {
                 const preset = document.getElementById("criteria_preset");
                 const chosen =
                   preset && preset.selectedIndex >= 0
                     ? preset.options[preset.selectedIndex]
                     : null;
                 const text = document.getElementById("criteria_text");
                 const preview = document.getElementById("criteria_preview");
                 return {
                   builder_present: !!document.getElementById("criteria_builder"),
                   preset: chosen ? chosen.textContent : null,
                   conditions: document.querySelectorAll(
                     "#criteria_rows .criteria-row"
                   ).length,
                   preview: preview ? preview.textContent : null,
                   textarea: text ? text.value : null,
                 };
               }"""
        )

    def choose_review_preset(self, label):
        "Pick a 'Start from' preset by its visible label."
        self.page.select_option("#criteria_preset", label=label)

    def set_review_condition_value(self, position, value):
        "Set the value control of the nth (1-based) condition row."
        row = self.page.locator("#criteria_rows .criteria-row").nth(position - 1)
        row.locator(".cb-value").select_option(value)

    def save_review_spec(self, name):
        "Name the spec, submit, and wait for the dashboard."
        self.page.fill("#review_spec_form #name", name)
        self.page.click("#review_spec_form button[type=submit]")
        self.page.wait_for_url("**/review/index")

    def get_review_spec_table(self):
