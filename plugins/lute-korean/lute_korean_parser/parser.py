...
Lute parser plugin using kiwipiepy (https://github.com/bab2min/kiwipiepy),
...
Configurable behaviour: the parser reads ``kiwi_*`` attributes from the
Language object when they exist (e.g. when running on a Lute fork that
adds these settings), and otherwise falls back to the defaults below:

* kiwi_tokenizer_mode:
    - 'morpheme' (default) : split into individual morphemes
                             (e.g. 예상 + 하 + 었 + 는데).
                             Recommended for new learners.
    - 'lemma'              : group morphemes of the same 어절 so that the
                             surface forms of predicate morphemes (V*/XSV)
                             are replaced with their dictionary lemma
                             (e.g. 예상했었는데 → 예상하다).
    - 'eojeol'             : entire 어절 (whitespace-delimited block) is
                             kept as a single token.

* kiwi_filter_particles (bool, default False):
...
class KoreanParser(AbstractParser):
    ...
    Text is split into tokens controlled by the kiwi_tokenizer_mode
    setting (when present on the language), defaulting to 'morpheme'.
    ...
    def get_lemma(self, text: str, language: Optional[object] = None):
        ...
        If the language has kiwi_stemming explicitly set to False,
        returns None.  Otherwise uses Kiwi's lemmatization so that:
            먹었어 → 먹다
            예상했었는데 → 예상하다
